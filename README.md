...
To configure your database, you'll first need to create two 'empty' databases using PostgreSQL (I recommend using psql inside your terminal). These databases should be titled 'witter' and 'witter_test' respectively. Once created, you'll want to add the schema from schema.sql into both databases. Assuming you are using psql within your terminal (and you are still in the directory containing the schema.sql file), use the commands 'psql witter < schema.sql' and 'psql witter_test < schema.sql'. Afterwards, try to access both databases and use the command '/dt' to see if the tables have been created (if successful, both databases should share the same number of tables).
